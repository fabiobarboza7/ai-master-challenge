...
| **E1** | Para comparar os quatro desafios, a IA montou uma taxa de aprovação a partir das reviews públicas — e contava "PR aguardando ajuste" como reprovado, o que infla a reprovação de quem simplesmente ainda não voltou | Revisor-IA apontou o problema de denominador | Denominador corrigido. A recomendação continuou a mesma, agora pelo motivo certo. Os números por desafio ficaram fora da entrega: são resultado de outros candidatos, não meu |
...
- **E2 — filtro de stage vazio.** `git ls-files --exclude-from` não listava nenhum arquivo. Pego num teste com `node_modules` e `.env` falsos, antes do primeiro commit. Virou um script que bloqueia o commit com dependência, dado bruto, `.env`, arquivo acima de 3 MB ou alteração fora da pasta. O remendo óbvio (`git add -f .`) teria posto `node_modules` no PR, que as reviews públicas apontam como falta de higiene.
